...
ax2.set_title("corrected image with pixel map function, azi_thresh:{}$^\circ$".format(thresh))
# pixel_map= pixel_map_func(calib_img, radius = 258, elevation_map_src=elevation_map_src, elevation_map_corr=elevation_map_corr, azimuth_map=azi_map)
ele_range = np.deg2rad(np.arange(0, 91, 30))
...
ax3.set_title("Equirect projection of original image")
plt.imshow(equi_plot_after)
plt.xticks(np.linspace(0,int(2 * np.pi * radius),9))
ax3.set_xticklabels(np.linspace(0,360,9,dtype="int"))

plt.yticks(np.linspace(0, radius, 4))
ax3.set_yticklabels(np.linspace(90, 0, 4,dtype="int"))
...
ax4.set_title("caliberated image expected longitude and latitutes ")
plt.imshow(sample_img_med)
# plt.imshow(sample_img_med,alpha=0.5)
plt.xticks(np.linspace(0,int(2 * np.pi * radius),9))
ax4.set_xticklabels(np.linspace(0, 360, 9, dtype="int"))

plt.yticks(np.linspace(0, radius, 4))
ax4.set_yticklabels(np.linspace(90, 0, 4,dtype="int"))
...
ax5.set_title("simple gaussian kernel , elevation :{}$^\circ$".format(ele_val))
plt5 = plt.imshow(kernel)
cbar = fig1.colorbar(plt5, ax=ax1, label ="weights")
# cbar.set_ticklabels(np.ceil(np.rad2deg(ele_range)))
...
ax6.set_title("sample tiling function in action , side_tile:{}$^\circ$, top_tile :{}$^\circ$"
...
ax6.set_xticklabels(azi_ticks_labels)
plt.yticks(ele_ticks)
ax6.set_yticklabels(ele_ticks_labels)


plt6 = plt.imshow(tiled_im)


plt.xlabel("azimuth in ($^\circ$)")
plt.ylabel("elevation in ($^\circ$)")
ax6.set_aspect(tiled_im.shape[1]/tiled_im.shape[0])
# plt.tight_layout()
