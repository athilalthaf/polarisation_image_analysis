"""
Script to visualise the bee ommatidial cordinates along with Dorsal Rim Area based on the paper
...
taken from invertpy project written by Evripidis    Gkanias

Just to plot the bee ommatidial cordinates for reference
...
def build_right_bee_eye():
    ...
    omm_pol = np.asarray(ommatidia[:, 1] > np.pi/3, dtype=float) #ommatidia above 60 degree elevation is considered as part of DRA
...
plt.title("ommatidial co-ordinates")
plt.xlim([-180, 180])
# plt.ylim([0, 90])
